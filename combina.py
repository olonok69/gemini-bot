...
def reload_page(st, model, embeddings, index, vectorstore, fname, df, placeholder):
    """
...
def main(model, embeddings, index, vectorstore, col1, col2):
    """
    main loop
                    ...
                    if seccion and st.session_state["section_prompt_selected"] == True:
                        ...
                        if len(r["matches"]) > 0:
                            ...
                            if (
                                ...
                                and st.session_state["chat_true"] == "no_chat"
                                and st.session_state["b_accept_inside_pericial"]
                                == False
                            ):
                                ...
                                file = (
                                    st.session_state["answer_introduced"].get(
                                        "filename"
                                    )
                                    + f" + prompt: {title}"
                                    + f" + Seccion: {seccion}"
